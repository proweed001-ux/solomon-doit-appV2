...
import { resolveProductMasters } from "./promo-product-master-sync.ts";
import {
  BUCKET,
  CLASS_SORT,
  corsHeaders,
...
Deno.serve(async req => {
  ...
  try {
    ...
    const prelim = cards.map(raw => {
      ...
      const masterScore = cleanConfidence(raw?.master_match_score), masterMargin = cleanConfidence(raw?.master_match_margin), titleConsensusScore = cleanConfidence(raw?.title_consensus_score);
      const masterIsNew = raw?.master_is_new === true, masterCanonicalName = cleanText(raw?.master_canonical_name, 200), masterNormalizedKey = cleanText(raw?.master_normalized_key, 240);
      ...
      return { raw, fileName, parsed, cardId, parsedFunction, label: parsedFunction.label, baseUnitPrice, unitLabel, masterProductId, masterScore, masterMargin, titleConsensusScore: titleConsensusScore || 0, masterIsNew, masterCanonicalName, masterNormalizedKey, fullPath: `${promoMonthId}/${parsed.classId}/${fileName}` };
    });
    ...
    const masterById = await resolveProductMasters(supabase, prelim, promoMonthId, now);
    ...
    const ids = normalized.map(item => item.cardId), groupIds = [...new Set(normalized.map(item => item.groupId))];
    ...
    const cardGroupRows = normalized.map(item => ({ promo_month_id: promoMonthId, card_id: item.cardId, group_id: item.groupId, decision: "MATCH", match_score: item.masterScore / 100, match_margin: item.masterMargin / 100, visual_score: cleanConfidence(item.raw?.visual_match_score) == null ? null : cleanConfidence(item.raw?.visual_match_score) / 100, title_score: item.masterScore / 100, size_score: null, price_image_score: cleanConfidence(item.raw?.price_confidence) == null ? null : cleanConfidence(item.raw?.price_confidence) / 100, reason: item.masterIsNew ? "Strong dual-title OCR created a deterministic product master" : "Product master + dual title OCR + group price consensus", adaptive: true, evidence: { master_product_id: item.masterProductId, canonical_name: item.promoTitle, master_is_new: item.masterIsNew, title_consensus_score: item.titleConsensusScore, title_ocr: cleanText(item.raw?.title_ocr, 500), price_support: Number(item.raw?.price_group_support || 0), visual_hash: cleanText(item.raw?.visual_title_hash, 100) }, updated_at: now }));
