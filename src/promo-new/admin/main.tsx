...
function AdminApp() {
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      ...
      <div className="footer-actions"><button className="btn primary" disabled={!STAGING_PREVIEW || !dataset || !session || demo || dryRun || busy || Boolean(savedVersionId) || dataset.version.status === 'published'} title={!STAGING_PREVIEW ? 'ปุ่มนี้เปิดเฉพาะ Preview ที่เชื่อมฐานทดสอบ และปิดบน Production' : 'บันทึกข้อมูลและรูปทั้งหมดเป็น Draft ในฐานทดสอบ'} onClick={save}><Save size={16} /> {savedVersionId ? '2/3 Draft บันทึกแล้ว' : '2/3 บันทึก Draft หน้าลูกค้า'}</button><button className="btn soft" disabled={!dataset || busy || (!dryRun && !savedVersionId)} title={!dataset ? 'ยังไม่มี Dataset' : busy ? 'กำลังทำงาน' : !dryRun && !savedVersionId ? 'บันทึก Draft สำหรับหน้าลูกค้าทดสอบก่อน' : ''} onClick={validatePreview}><CheckCircle2 size={16} /> {previewChecked ? 'ตรวจผ่านแล้ว' : 'ตรวจความพร้อม'}</button><button className="btn dark" disabled={!publishable} title={!publishable ? 'ต้องบันทึก Draft และผ่านการตรวจความพร้อมก่อน; Production ยังถูกปิด' : 'Publish ไปหน้าลูกค้าทดสอบเท่านั้น'} onClick={publish}>3/3 Publish หน้าลูกค้าทดสอบ</button></div>
    </main>
  </div>;
}
