(()=>{
  ...
  function preloadQrFast(url){try{const i=new Image();i.decoding='async';i.src=url||QR_STABLE}catch{}}
  ...
  function normalize(c){if(!c)return null;const x={...c};if(x.enabled!==false)x.enabled=true;if(!x.image_url||String(x.image_url).includes('/storage/v1/object/public/doit-files/team/dev-qr.png'))x.image_url=QR_IMAGE+'&t='+encodeURIComponent(x.updated_at||'stable');return x}
  async function loadQr(){if(qrCache)return qrCache;if(qrPromise)return qrPromise;qrPromise=(async()=>{try{const r=await fetch(QR_CONFIG,{cache:'no-store'});if(!r.ok)throw Error(String(r.status));qrCache=normalize(await r.json());try{localStorage.setItem('doit-dev-qr-config-v1',JSON.stringify(qrCache))}catch{}}catch{try{qrCache=normalize(JSON.parse(localStorage.getItem('doit-dev-qr-config-v1')||'null'))}catch{qrCache=null}}if(!qrCache)qrCache=normalize({image_url:QR_STABLE});return qrCache})();return qrPromise}
  async function primeQr(){qrBlock();let cached=null;try{cached=normalize(JSON.parse(localStorage.getItem('doit-dev-qr-config-v1')||'null'))}catch{}if(cached){preloadQrFast(cached.image_url);renderQr(cached)}else if(!qrRendered)renderQr(normalize({image_url:QR_STABLE}));const cfg=await loadQr();if(cfg)renderQr(cfg)}
