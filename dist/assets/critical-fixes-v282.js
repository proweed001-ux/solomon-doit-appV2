(()=>{'use strict';
// Critical field-test patches for Pro JSON mode.
// Keep this file small and loaded after existing overlays.
try{if(typeof window.sshape==='undefined')window.sshape=false}catch{}
...
function refreshApp(){try{try{window.sshape=false}catch{}document.body.classList.remove('printMode');$('#doitPrintOverlay')?.remove();if(lastPayload&&window.DOIT_JSON_APP?.load){window.DOIT_JSON_APP.load(lastPayload,lastMeta)}else{document.dispatchEvent(new CustomEvent('doit:stateChanged'))}}catch(e){console.warn('[critical-fixes-v282] refresh failed',e)}}
...
function boot(){try{window.sshape=false}catch{}patchJsonApp();patchTeleDrawer();bindCloseButtons();let n=0;const tick=()=>{try{window.sshape=false}catch{}patchJsonApp();patchTeleDrawer();if(++n<30)setTimeout(tick,300)};setTimeout(tick,300)}
