...
function updatePreview(){restoreAllDates();const arr=selectedDates(),p=$('#datePreview');if(!p||!arr.length)return;if(arr.length===1)p.textContent=thai(arr[0]);else p.textContent=`เลือก ${arr.length} วัน: ${thai(arr[0])} - ${thai(arr[arr.length-1])}`}
function patchApp(){const app=window.DOIT_JSON_APP;if(!app||app.__selectedDateScope301)return false;const oldLoad=app.load.bind(app);app.load=(payload,m={})=>{rememberAllDates(payload);if(!loading){rawPayload=payload;rawMeta=m||{}}const next=filteredPayload(payload);const result=oldLoad(next,m);restoreAllDates();setTimeout(updatePreview,0);setTimeout(updatePreview,150);setTimeout(restoreAllDates,300);return result};app.rerender=()=>{if(!rawPayload)return;loading=true;try{app.load(rawPayload,rawMeta)}finally{loading=false}restoreAllDates();setTimeout(updatePreview,0);setTimeout(updatePreview,150)};app.__selectedDateScope301=true;patched=true;return true}
function onDatesChanged(){selectedDates();restoreAllDates();if(!patched)patchApp();if(window.DOIT_JSON_APP?.rerender)window.DOIT_JSON_APP.rerender();else updatePreview()}
function boot(){patchApp();restoreAllDates();updatePreview();let n=0;const tick=()=>{patchApp();restoreAllDates();updatePreview();if(++n<40)setTimeout(tick,300)};setTimeout(tick,300)}
