...
function forceSelect(v){const el=document.querySelector('#psSelect');if(!el||!v)return;let o=[...el.options].find(x=>x.value===v);if(!o){o=document.createElement('option');o.value=v;o.textContent=v;el.appendChild(o)}if(el.value!==v){el.value=v;el.dispatchEvent(new Event('change',{bubbles:true}))}}
let rawPayload=null,rawMeta={},rawRows=[];
function patch(){const app=window.DOIT_JSON_APP;if(!app||app.__multiPsBridge310)return false;const old=app.load.bind(app);app.load=(payload,m={})=>{const arr=rowsOf(payload);if(arr.length&&!String(arr[0]?.ps||'').startsWith('MULTI: ')){rawPayload=payload;rawMeta=m||{};rawRows=arr;window.__doitAllRows=arr;window.__doitRawRows310=arr}const ps=selected();let next=payload;if(ps.length){const set=new Set(ps),key=scopeKey(ps);const src=rowsOf(payload);next=withRows(payload,src.filter(r=>set.has(cleanPs(r.ps))).map(r=>({...r,ps:key})));forceSelect(key)}return old(next,m)};app.__multiPsBridge310=true;return true}
