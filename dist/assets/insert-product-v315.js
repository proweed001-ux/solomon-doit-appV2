...
function customKey(){return 'doit-insert-products-v315:'+appKey()}
function legacyKeys(){return['doit-insert-products-v314:'+appKey(),'doit-insert-products-v313:'+appKey(),'doit-insert-products-v309:'+appKey()]}
...
function addButton(){const a=document.querySelector('.actions');if(!a)return;let b=$('#insertProductBtn315')||$('#insertProductBtn314')||$('#insertProductBtn313')||$('#insertProductBtn309');if(!b){b=document.createElement('button');b.id='insertProductBtn315';b.type='button';b.className='outline';b.textContent='+ แทรกสินค้า';a.prepend(b)}b.id='insertProductBtn315';b.onclick=null;b.dataset.insertVersion='v315'}
...
const arr=customRows().filter(x=>T(x.id));arr.push({id:Date.now(),date:sc.date,ps:sc.ps,name,qty,unit,code,brand:'แทรกสินค้า',size:'เพิ่มเอง'});write(customKey(),arr);msg('แทรกสินค้าแล้ว: '+name+' จำนวน '+qty);rerender()}finally{setTimeout(()=>{inserting=false},500)}}
function rerender(){if(!ensurePayload()||!window.DOIT_JSON_APP)return;const scope=rememberScope();try{window.DOIT_JSON_APP.load(basePayload,baseMeta)}catch(e){console.warn('[insert-product-v315] rerender failed',e)}restoreScope(scope);setTimeout(removeFakeStore,120);setTimeout(addButton,150)}
function patch(){const app=window.DOIT_JSON_APP;if(!app||app.__insertProduct315)return false;const old=app.load.bind(app);app.load=(payload,m={})=>{const clean=cleanPayload(payload);basePayload=clean;baseMeta=m||{};const res=old(augment(clean),m);setTimeout(addButton,80);setTimeout(removeFakeStore,120);return res};app.__insertProduct315=true;patched=true;return true}
function captureClick(e){const b=e.target&&e.target.closest&&e.target.closest('#insertProductBtn315,#insertProductBtn314,#insertProductBtn313,#insertProductBtn309');if(!b)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();insertProduct()}
function boot(){patch();addButton();removeFakeStore();if(!document.__insertProduct315Capture){document.__insertProduct315Capture=true;document.addEventListener('click',captureClick,true)}let n=0;const tick=()=>{patch();addButton();ensurePayload();removeFakeStore();if(++n<900)setTimeout(tick,1000)};setTimeout(tick,300)}
window.DOIT_INSERT_PRODUCT={insertProduct,customRows,customKey,rerender,health:()=>({version:'v315',patched,customKey:customKey(),count:customRows().length,baseRows:rowsOf(basePayload).length,globalRows:getRowsFromGlobals().length})};
