...
function patchApp(app){if(!app||!app.load||app.__stable311)return app;const old=app.load.bind(app);app.load=(payload,m={})=>{const clean=enrichPayload(payload);const res=old(clean,m);setTimeout(refreshTele,80);setTimeout(refreshTele,450);return res};app.__stable311=true;return app}
function installAppOwner(){try{if(Object.getOwnPropertyDescriptor(window,'DOIT_JSON_APP')?.set)return;appRef=window.DOIT_JSON_APP||appRef;Object.defineProperty(window,'DOIT_JSON_APP',{configurable:true,get(){return appRef},set(v){appRef=patchApp(v)}});if(appRef)appRef=patchApp(appRef)}catch{if(window.DOIT_JSON_APP)patchApp(window.DOIT_JSON_APP)}}
...
window.DOIT_PRO_STABILITY={version:'v311',enrichRow,enrichPayload,refreshTele,health:()=>({stableRows:stableRows.length,rows:rows().length,drawer:!!window.DOIT_TELESALE_DRAWER,json:!!window.DOIT_JSON_APP,appPatched:!!window.DOIT_JSON_APP?.__stable311})};
