...
function enrichPayload(p){try{const rows=p?.rows||p;if(!Array.isArray(rows))return p;const out=rows.map(enrichRow);stableRows=out;window.__doitStableRows311=out;window.__doitAllRows=out;return Array.isArray(p)?out:{...p,rows:out}}catch{return p}}
function markOwner(app){try{app.__fieldLogic295=true;app.__fieldLogic308=true;app.__filterCoordinatorStoreUnderPs297=true;app.__psScopeOwner295=true;app.__appCoordinator290=true;app.__teleForceRefresh310=true;app.__teleStrict310=true;app.__stableOwner311=true}catch{}return app}
function patchApp(app){if(!app||!app.load)return app;markOwner(app);if(app.__stable311)return app;const old=app.load.bind(app);app.load=(payload,m={})=>{const clean=enrichPayload(payload);const res=old(clean,m);setTimeout(refreshAll,80);setTimeout(refreshAll,450);return res};app.__stable311=true;return app}
...
function rows(){const c=[stableRows,window.__doitStableRows311,window.__doitAllRows,window.__doitRawRows304,window.__doitRawRows290,window.__doitPsRawRows295];for(const r of c){if(Array.isArray(r)&&r.length)return r.map(enrichRow)}return[]}
...
function refreshStore(){try{window.DOIT_STORE_PS_FIX?.update?.()}catch(e){console.warn('[pro-stability-v311] store refresh failed',e)}}
function refreshAll(){refreshTele();refreshStore()}
...
function bindEvents(){if(document.__stable311Events)return;document.__stable311Events=true;['doit:scopeRendered','doit:psChanged','doit:selectedDatesChanged'].forEach(ev=>document.addEventListener(ev,()=>setTimeout(refreshAll,0)));document.addEventListener('click',e=>{if(e.target?.id==='cloudLoadBtn')setTimeout(()=>{installAppOwner();refreshAll()},1200)},true)}
function boot(){lockTitle();installAppOwner();bindEvents();refreshAll();let n=0;const tick=()=>{lockTitle();installAppOwner();refreshAll();if(++n<20)setTimeout(tick,350)};setTimeout(tick,150)}
window.DOIT_PRO_STABILITY={version:'v316',enrichRow,enrichPayload,refreshTele,refreshAll,teleStats,health:()=>({stableRows:stableRows.length,rows:rows().length,drawer:!!window.DOIT_TELESALE_DRAWER,json:!!window.DOIT_JSON_APP,appPatched:!!window.DOIT_JSON_APP?.__stable311,owner:!!window.DOIT_JSON_APP?.__stableOwner311,tele:teleStats()})};
