...
function bindButtons(){const help=$('#helpBtn');if(help&&!help.dataset.help305){help.dataset.help305='1';help.onclick=()=>alert('วิธีใช้งานย่อ\n1) โหลดไฟล์ล่าสุดจาก Cloud\n2) เลือกวันที่/PS ที่ต้องการ\n3) เลือกร้านจริงที่จะส่งของ\n4) ใส่จำนวนส่งจริงในตาราง\n5) กดเตรียมปริ้น แล้วตรวจราคาก่อนสั่งปริ้น')}
const auto=$('#autosaveBtn');if(auto&&!auto.dataset.auto305){auto.dataset.auto305='1';auto.onclick=()=>{const m=$('#msg');if(m)m.textContent='บันทึกอัตโนมัติทำงานแล้ว · '+new Date().toLocaleTimeString('th-TH')}}}
function bind(){if(document.__filterCoord305)return;document.__filterCoord305=true;document.addEventListener('click',e=>{if(e.target?.id==='clearFilter'){e.preventDefault();e.stopImmediatePropagation();resetAllFilters()}},true);document.addEventListener('doit:selectedDatesChanged',()=>setTimeout(()=>{syncGlobals();window.DOIT_JSON_APP?.rerender?.();afterRender()},0));document.addEventListener('change',e=>{if(['storeSelect','brandSelect','typeSelect','q'].includes(e.target?.id))setTimeout(afterRender,0)},true)}
function boot(){patchApp();bind();bindButtons();afterRender();let n=0;const tick=()=>{patchApp();bind();bindButtons();afterRender();if(++n<12)setTimeout(tick,500)};setTimeout(tick,300)}
