...
function scopedRows(){const ex=excludedStores();return sourceScopeRows().filter(r=>!ex.has(T(r.store)))}
...
function afterRender(){syncGlobals();updatePreview();updateStoreOptions();removeDebug();document.dispatchEvent(new CustomEvent('doit:scopeRendered'))}
function patchApp(){const app=window.DOIT_JSON_APP;if(!app||app.__filterCoordinatorStoreUnderPs297)return false;const oldLoad=app.load.bind(app);app.load=(payload,m={})=>{remember(payload,m);syncGlobals();const next=rawRows.length?clone(rawPayload||payload,scopedRows()):payload;const res=oldLoad(next,m);setTimeout(afterRender,0);setTimeout(afterRender,150);return res};app.rerender=()=>{if(!rawPayload)return;app.load(rawPayload,rawMeta)};app.__filterCoordinatorStoreUnderPs297=true;return true}
...
function bindHints(){const help=$('#helpBtn');if(help&&!help.dataset.helpStore297&&!help.onclick){help.dataset.helpStore297='1';help.addEventListener('click',()=>alert('วิธีใช้งานย่อ\n1) โหลดไฟล์ล่าสุดจาก Cloud\n2) เลือกวันที่/PS ที่ต้องการ\n3) ใต้ PS ใช้กรองร้านออเดอร์เพื่อตัดบางร้านออกจากตารางถอดของ\n4) เลือกร้านจริงที่จะส่งของ\n5) ใส่จำนวนส่งจริง แล้วเตรียมปริ้น'))}const auto=$('#autosaveBtn');if(auto&&!auto.dataset.autoStore297){auto.dataset.autoStore297='1';auto.addEventListener('click',()=>setTimeout(()=>{const m=$('#msg');if(m&&!/บันทึก/.test(m.textContent||''))m.textContent='บันทึกอัตโนมัติพร้อมใช้งาน · '+new Date().toLocaleTimeString('th-TH')},30))}}
function bind(){if(document.__filterCoordStoreUnderPs297)return;document.__filterCoordStoreUnderPs297=true;document.addEventListener('click',e=>{if(e.target?.id==='clearFilter'){e.preventDefault();e.stopImmediatePropagation();resetAllFilters()}},true);document.addEventListener('doit:selectedDatesChanged',()=>setTimeout(()=>{syncGlobals();window.DOIT_JSON_APP?.rerender?.();afterRender()},0));document.addEventListener('doit:orderStoreFilterChanged',()=>setTimeout(()=>{syncGlobals();window.DOIT_JSON_APP?.rerender?.();afterRender()},0));document.addEventListener('change',e=>{if(['storeSelect','brandSelect','typeSelect','q'].includes(e.target?.id))setTimeout(afterRender,0)},true)}
