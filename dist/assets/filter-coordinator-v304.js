...
function bindHints(){const help=$('#helpBtn');if(help&&!help.dataset.help305b&&!help.onclick){help.dataset.help305b='1';help.addEventListener('click',()=>alert('วิธีใช้งานย่อ\n1) โหลดไฟล์ล่าสุดจาก Cloud\n2) เลือกวันที่/PS ที่ต้องการ\n3) เลือกร้านจริงที่จะส่งของ\n4) ใส่จำนวนส่งจริงในตาราง\n5) กดเตรียมปริ้น แล้วตรวจราคาก่อนสั่งปริ้น'))}const auto=$('#autosaveBtn');if(auto&&!auto.dataset.auto305b){auto.dataset.auto305b='1';auto.addEventListener('click',()=>setTimeout(()=>{const m=$('#msg');if(m&&!/บันทึก/.test(m.textContent||''))m.textContent='บันทึกอัตโนมัติพร้อมใช้งาน · '+new Date().toLocaleTimeString('th-TH')},30))}}
function bind(){if(document.__filterCoord305b)return;document.__filterCoord305b=true;document.addEventListener('click',e=>{if(e.target?.id==='clearFilter'){e.preventDefault();e.stopImmediatePropagation();resetAllFilters()}},true);document.addEventListener('doit:selectedDatesChanged',()=>setTimeout(()=>{syncGlobals();window.DOIT_JSON_APP?.rerender?.();afterRender()},0));document.addEventListener('change',e=>{if(['storeSelect','brandSelect','typeSelect','q'].includes(e.target?.id))setTimeout(afterRender,0)},true)}
