...
let rawPayload=null,rawRows=[],patched=false,cleaned=false;
...
function cleanUnstableInsert(){if(cleaned)return;cleaned=true;keys('doit-manual-v1:').forEach(k=>localStorage.removeItem(k));keys('doit-insert-rows-v1:').forEach(k=>localStorage.removeItem(k));document.querySelector('#manualAddBtn')?.remove();document.querySelector('#insertRowBtn290')?.remove();document.querySelectorAll('.manualOrderRow290,.insertRow290').forEach(x=>x.remove())}
function patchLoad(){const app=window.DOIT_JSON_APP;if(!app||app.__appCoordinator290)return false;const old=app.load.bind(app);app.load=(payload,m={})=>{const arr=rowsOf(payload);if(Array.isArray(arr)&&arr.length){rawPayload=payload;rawRows=arr;window.__doitRawRows290=arr;window.DOIT_ALL_DATES=dates();window.DOIT_AVAILABLE_DATES=window.DOIT_ALL_DATES}cleanUnstableInsert();const res=old(payload,m);setTimeout(syncUi,0);setTimeout(syncUi,250);setTimeout(cleanUnstableInsert,300);return res};app.__appCoordinator290=true;patched=true;return true}
function syncUi(){lockTitle();cleanUnstableInsert();const ds=selectedDates();const p=$('#datePreview');if(p){if(!ds.length)p.textContent='ทุกวัน';else if(ds.length===1)p.textContent=thai(ds[0]);else p.textContent=`เลือก ${ds.length} วัน: ${thai(ds[0])} - ${thai(ds[ds.length-1])}`}const pb=$('#psTickBtn289');if(pb){const ps=selectedPs();pb.textContent=ps.length?`PS: ${ps.length===1?ps[0]:ps.length+' คน'} ▼`:'PS: ทั้งหมด ▼'}window.DOIT_APP_SCOPE=getScope()}
function bindCleanup(){if(document.__disableInsert290)return;document.__disableInsert290=true;document.addEventListener('click',e=>{const b=e.target?.closest?.('#manualAddBtn,#insertRowBtn290');if(b){e.preventDefault();e.stopImmediatePropagation();b.remove()}},true);['doit:scopeRendered','doit:selectedDatesChanged','doit:psChanged'].forEach(ev=>document.addEventListener(ev,()=>{cleaned=false;setTimeout(cleanUnstableInsert,60)}))}
function health(){return{version:APP_VERSION,patched,insertDisabled:true,rows:rawRows.length,scope:getScope()}}
function expose(){window.DOIT_APP_COORDINATOR={version:APP_VERSION,getScope,health,rerender:()=>window.DOIT_JSON_APP?.rerender?.()}}
function boot(){cleanUnstableInsert();patchLoad();bindCleanup();expose();syncUi();let n=0;const tick=()=>{cleaned=false;patchLoad();bindCleanup();expose();syncUi();if(++n<10)setTimeout(tick,600)};setTimeout(tick,300)}
