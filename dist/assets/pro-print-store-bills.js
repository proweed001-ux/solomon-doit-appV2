(() => {
  ...
  const DOWN = value => Math.floor(N(value)).toLocaleString('th-TH');
  ...
  function textValue(selector, label) {
    const raw = T($(selector)?.textContent).replace(new RegExp('^' + label + ':\\s*'), '');
    if (!raw || raw.includes('ทั้งหมด') || raw.includes('เลือก') || raw.includes('ยังไม่เลือก')) return '';
    return raw;
  }

  function currentPs() {
    return textValue('#psText', 'PS');
  }

  function currentReceivers() {
    try {
      const health = window.DOIT_CORE_APP?.health?.() || {};
      if (Array.isArray(health.receivers) && health.receivers.length) return health.receivers.filter(Boolean);
    } catch {}
    const text = textValue('#sendText', 'ส่งให้ร้าน');
    return text ? [text] : [];
  }
  ...
  function getState() {
    ...
    keys.forEach(key => {
      try {
        ...
        const manualCount = Object.keys(state.send || {}).length
        ...
        const score = psScore + receiverScore + manualCount;
        if (score > bestScore) {
          bestScore = score;
          best = state;
        }
  ...
  function rowsForStore(state, store, pool) {
    ...
    return rows.filter(row => T(row.name) && N(row.qty) > 0);
  }
  ...
  function lineEditKey(state, store, row) {
    return [editScope(state), store || '', row.code || '', row.name || ''].join(SEP);
  }
  ...
  function lineValues(state, store, row) {
    const edit = getLineEdit(lineEditKey(state, store, row));
  ...
  function buildBills() {
    ...
    stores.forEach((store, storeSeq) => {
      ...
      const storeRawTotal = rows.reduce((sum, row) => sum + lineValues(state, store, row).total, 0);
      const storeTotal = Math.floor(storeRawTotal);
  ...
  function blankRows(count) {
    return Array.from({ length: Math.max(0, BILL_ROWS - count) }, () => (
  ...
  function receiptHtml(bill, index) {
    ...
    const body = bill.rows.map((row, rowIndex) => {
      const key = lineEditKey(bill.state, bill.store, row);
      const values = lineValues(bill.state, bill.store, row);
      const cells = [row.code, row.name, F(values.qty), B(values.unit), B(values.total)];
      return `<tr data-line data-edit-key="${E(ENC(key))}"><td class="c">${(bill.startNo || 1) + rowIndex}</td>${cells.map((value, cellIndex) => `<td class="${cellIndex >= 2 ? 'r' : ''} ${cellIndex === 1 ? 'itemName' : ''} ${cellIndex === 2 ? 'rq' : ''} ${cellIndex === 3 ? 'ru' : ''} ${cellIndex === 4 ? 'rt' : ''}" contenteditable="${cellIndex === 4 ? 'false' : 'true'}">${E(value)}</td>`).join('')}</tr>`;
    }).join('');
    const totalLabel = bill.isLastStorePart ? 'รวมทั้งหมด' : `ต่อใบถัดไป (${bill.partNo || 1}/${bill.partCount || 1})`;
    const totalValue = bill.isLastStorePart ? DOWN(bill.storeTotal) : '';
  ...
  function recalcReceiptPage(page, persist = false) {
    ...
    root.querySelectorAll(`.receiptPage[data-store-seq="${seq}"] tr[data-line]`).forEach(row => {
      rawTotal += N(row.querySelector('.rq')?.textContent) * N(row.querySelector('.ru')?.textContent);
    });
    totalEl.textContent = DOWN(rawTotal);
  }
  ...
  function doneSummary() {
    ...
    bills.forEach(bill => {
      if (!storeMap.has(bill.storeSeq)) {
        storeMap.set(bill.storeSeq, { store: bill.store, total: bill.storeTotal, billCount: bill.partCount || 1 });
      }
      bill.rows.forEach(row => {
        ...
        item.qty += values.qty;
        item.rawTotal += values.total;
      });
    });

    return {
      products: [...productMap.values()].sort((a, b) => T(a.name).localeCompare(T(b.name), 'th')),
      storeTotal: [...storeMap.values()].reduce((sum, store) => sum + Math.floor(N(store.total)), 0),
    };
  }

  function renderDoneFromBills() {
    ...
    const signature = JSON.stringify(products.map(item => [item.name, item.qty, item.rawTotal]).concat([['TOTAL', summary.storeTotal]]));
    if (table.dataset.proPrintDoneSignature === signature) return;
    table.dataset.proPrintDoneSignature = signature;

    tableCount.textContent = `จัดแล้ว ${F(products.length)} รายการ · รวมจากบิลเตรียมปริ้น ${DOWN(summary.storeTotal)} บาท`;
    table.innerHTML = `<thead><tr><th>#</th><th>สินค้า</th><th>จำนวนที่ส่งทั้งหมด</th><th>ราคารวมส่ง</th></tr></thead><tbody>${products.length ? products.map((item, index) => `<tr><td>${index + 1}</td><td>${E(item.name)}</td><td>${F(item.qty)}</td><td>${B(item.rawTotal)}</td></tr>`).join('') + `<tr class="totalRow"><td colspan="3" class="r">รวมจากบิลเตรียมปริ้น</td><td class="r">${DOWN(summary.storeTotal)}</td></tr>` : '<tr><td colspan="4" class="empty">ไม่มีข้อมูล</td></tr>'}</tbody>`;
  }

  function scheduleDoneRender() {
    setTimeout(renderDoneFromBills, 60);
  }
  ...
  function install() {
    ...
    const tick = () => {
      ...
      if (button && button.dataset.printStoreBillsInstalled !== '1') {
        button.dataset.printStoreBillsInstalled = '1';
        button.addEventListener('click', handlePrep, true);
      }
    ...
    document.addEventListener('click', event => {
      if (event.target.closest('.tab') || event.target.closest('#searchBtn') || event.target.closest('#pickOk') || event.target.closest('#clearFilter')) scheduleDoneRender();
    }, true);
    document.addEventListener('input', event => {
      if (event.target.closest('#table')) scheduleDoneRender();
    }, true);
    scheduleDoneRender();
  }
