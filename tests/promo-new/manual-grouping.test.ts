...
import { assignCardsManually } from '../../src/promo-new/domain/manual-grouping';
...
test('manual grouping refuses duplicate Class', () => {
  ...
  assert.throws(
    () => assignCardsManually(dataset, [], [grouped.groups[1].cardIds[0]], { groupId: grouped.groups[0].id }),
    /Class HFSS ซ้ำกัน/,
  );
});
