...
test('ประกอบรูปการ์ดที่จัดกลุ่มกลับด้วยหน้าและลำดับแม้ Class recovery เปลี่ยน Card ID', () => {
...
test('ประกอบรูปการ์ด quarantine กลับด้วยหน้าและลำดับแม้ Card ID เปลี่ยน', () => {
  const image = 'data:image/png;base64,IMAGE';
  const prepared = prepareGroupingWorkerCards([card('OLD-ID', image)]);
  const grouped = {
    cards: [],
