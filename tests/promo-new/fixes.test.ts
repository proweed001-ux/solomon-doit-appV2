...
test('โหลด SKU และราคาเดิมแล้วจับ Family ที่ตรงครบ Class อัตโนมัติ', () => {
  ...
  assert.equal(result.dataset.productGroups[0].promotionFamilyId, 'family:pantene-70');
  assert.equal(result.dataset.productGroups[0].price.effectivePrice?.amount, 19.75);
  assert.equal(result.dataset.productGroups[0].status, 'ready');
  assert.deepEqual(result.dataset.cards.map(card => card.status), ['ready', 'ready']);
});
