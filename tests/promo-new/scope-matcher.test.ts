...
test('ภาพข้าม Class ช่วยกู้ชื่อที่อ่านไม่ได้เมื่อราคาขายปลีกตรงกับการ์ดอ้างอิง', () => {
  const promo = family('family:vicks-5', 'วิคส์ ขนาด 5 กรัม', ['HFSS', 'HFSM']);
  const reference = imported('VICKS Vaporub 5 กรัม', 'HFSS', 1);
  reference.rawText = 'ราคาแนะนำขายปลีก 30 บาท/ชิ้น VICKS Vaporub 5 กรัม';
  const missing = imported('', 'HFSM', 2);
  missing.rawText = 'ราคาแนะนำขายปลีก 30 บาท/ชิ้น';
  const signatures = {
...
test('ภาพเหมือนกันแต่ไม่มีแบรนด์ ขนาด รุ่น หรือราคาที่ตรงต้องไม่ข้ามกลุ่ม', () => {
  const vicks = family('family:vicks-5', 'วิคส์ ขนาด 5 กรัม', ['HFSS', 'HFSM']);
  const reference = imported('VICKS Vaporub 5 กรัม', 'HFSS', 1);
  const unknown = imported('', 'HFSM', 2);
  const same = repeatedHex(80);
  const resolutions = resolveScopesWithVisualConsensus([reference, unknown], [vicks], {
    [reference.cardId]: same,
    [unknown.cardId]: same,
  });
...
test('Safeguard PDF 120 กรัมจับ Family 125 กรัมได้แต่ถูกกักให้ยืนยันความขัดแย้งของขนาด', () => {
  const promo = family('family:safeguard-large', 'เซฟการ์ด สบู่ก้อน 125 กรัม', ['HFSM', 'HFSL']);
  const grouped = groupImportedCards(MONTH, [
    imported('Safeguard สบู่ก้อน ขนาด 120 กรัม', 'HFSM', 1),
    imported('เซฟการ์ด สบู่ก้อน 120 กรัม', 'HFSL', 2),
  ], [], [], [promo]);
  assert.equal(grouped.groups.length, 1);
  assert.equal(grouped.groups[0].promotionFamilyId, promo.id);
  assert.equal(grouped.groups[0].sku.identity.sizeValue, 120);
  assert.equal(grouped.groups[0].sku.status, 'quarantine');
  assert.ok(grouped.groups[0].sku.failureReasons.includes('scope_size_source_conflict'));
});
