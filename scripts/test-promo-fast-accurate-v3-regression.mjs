...
const templates=[
 ...
 '1-2 ชิ้น ลด 24%',
 '12 ชิ้น ลด 25%',
 'ลด 24%','ลด 25%','ลด 12%','ลด 27%'
];
const thaiDigits={'๐':'0','๑':'1','๒':'2','๓':'3','๔':'4','๕':'5','๖':'6','๗':'7','๘':'8','๙':'9'};
function cleanOcr(raw){let s=String(raw||'').replace(/[๐-๙]/g,d=>thaiDigits[d]||d).replace(/[|_=~—–]+/g,' ').replace(/ล\s*ด/g,'ลด').replace(/ฟ\s*รี/g,'ฟรี').replace(/ซื\s*้\s*อ/g,'ซื้อ');s=s.replace(/\b(?:an|aN|aA|AA|A4|4A|a4)\s*(?=\d+\s*%)/gi,'ลด ').replace(/\b(?:WS|ws)\s*(?=\d+)/g,'ฟรี ').replace(/(?:ข\s*บ?\s*วด|[ยบผ]วด|\b(?:UOA|UDA|YOA|YOM)\b)/gi,'ขวด').replace(/(?:ช\s*ิ?\s*้?\s*น|\b(?:Bu|GU|Gu|6u|bu|ชีน|ซิน|ขิน|ชน)\b)/gi,'ชิ้น').replace(/(?:แพ\s*็?\s*ค|\b(?:แพค|เเพ็ค|ulin)\b)/gi,'แพ็ค').replace(/(?:ล|ส)\s*ั?\s*ง/g,'ลัง').replace(/ก\s*ล\s*่?\s*อ\s*ง/g,'กล่อง').replace(/(\d{1,2})\s*[°º]/g,'$1%').replace(/\bB%/gi,'8%').replace(/ลด\s*(\d{1,2})9\b/g,'ลด $1%');return s.replace(/\s+/g,' ').trim()}
...
console.log(JSON.stringify({ok:true,parserAndBadgeCases:10,retryRequests:24},null,2));
